...
Once the game is started, it will send various messages with data such as [`player` information](), [`ui` messages](), [`msgs` from the log] and [`map` data](map.qmd) and the game will accept any of the standard keyboard inputs.